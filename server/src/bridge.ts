...
export class Bridge {
  ...
  handleUpgrade(request: IncomingMessage, socket: Duplex, head: Buffer): void {
    ...
    const fileKey = url.searchParams.get("fileKey") || this.nextLegacyFileKey();
  ...
  handleConnection(ws: WebSocket, fileKey: string, fileName = "Unknown"): void {
  ...
  private resolveConnection(fileKey?: string): WebSocket {
    ...
    throw new Error(
      `Multiple files connected. Specify a fileKey to choose which file to query. Connected files: ${files
        .map((file) => `"${file.fileName}" (fileKey: ${file.fileKey})`)
        .join(", ")}. Use the list_files tool to see all connected files.`
    );
  }

  private nextLegacyFileKey(): string {
    this.legacyCounter += 1;
    return `legacy-${this.legacyCounter}`;
  }
